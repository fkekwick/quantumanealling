...
##################################################
sampler = DWaveSampler(solver='DW_2000Q_6',token=) # 2000Q solver,
...
#encode h and J using Ising model

J = { (0,0):0, (0,1):1, (0,2):1, (0,3):0, (0,4):0,
(1,2):1, (1,4):1,
(2,3):1, (2,4):1 }   

h = [-1.5,-2.5,-3.5,-0.5,-1.5]  # h is a function of k, here k = 0.5
...
results = sampler_embedded.sample_ising(h, J, 
    num_reads=runs,
    annealing_time=200)  #vary annealing time
...
energy = results.record.energy  #record energy of each sample
sample = results.record.sample.tolist()    #record state of each sample
num_occurrences= results.record.num_occurrences #record number of occurences of each sample
...
#record results in a pandas data file also 
